...
describe('authorization matrix (every route Plan 5 does not already cover)', () => {
  ...
  const MATRIX: Row[] = [
    ...
    {
      label: 'admin book order create: student',
      method: 'post',
      path: () => '/api/admin/book-orders',
      actor: 'student',
      body: () => ({
    ...
    {
      label: 'admin book order create: admin, unknown course',
      method: 'post',
      path: () => '/api/admin/book-orders',
      actor: 'admin',
      // Reaches `BookOrdersService.adminCreate`, which 404s a course id that
      // does not exist — proof the `book-order:create` gate runs and the
      // route reaches real business logic, not a real order (that is
      // `book-orders.service.spec.ts`'s job).
      body: () => ({
        courseId: randomUUID(),
        fullName: 'عميل',
        phone: '+201012345678',
