...
export class CampaignRunner {
  ...
  private async step(): Promise<void> {
    ...
    /*
     * `null` on a PLATFORM-ONLY campaign, and that is the honest value: no
     * stanza was ever handed to WhatsApp, so there is no id a receipt could
     * ever match. Writing one would make a message that cannot be acknowledged
     * look like one still waiting to be — and `blindSend` counts exactly that.
     */
