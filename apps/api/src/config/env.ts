...
const schema = z
  .object({
    ...
    /**
     * محتوى ملف كوكيز يوتيوب (صيغة Netscape)، أو مش موجود خالص.
     *
     * من ٢٣ سبتمبر ٢٠٢٦ يوتيوب بيرد «Sign in to confirm you're not a bot» على
     * **كل** عملاء `YT_CLIENTS` الخمسة من IP السيرفر ده — يعني سلسلة تبديل
     * العميل مابقاش تحتها حاجة تقع عليها، والبوكِت فضل فاضي و١٤ صف في
     * `lesson_videos` كلهم `failed`. ده اللي `yt-dlp` نفسه بيسمّيه في رسالة
     * الخطأ، وهو الرافعة الوحيدة اللي في إيدينا: الرفض بتاع **العنوان** اللي
     * بنكلّم منه، وإحنا مش قادرين نغيّره.
     *
     * ⚠️ **مش** واحد من الخمسة بتوع «كلهم أو ولا واحد». الخمسة دول كده لأن
     * نصّهم = بوكِت محدش يقدر يقراه؛ ده تحسين صافي لما يبقى موجود ومابيكلّفش
     * حاجة لما يغيب — من غيره المرآة بتشتغل بالظبط زي ما كانت، وعلى جهاز
     * يوتيوب مش زعلان منه (لابتوب، CI، خط منزلي) لسه بتنجح.
     *
     * ⚠️ الكوكيز بتنتهي، وحساب بيتقرا من داتاسنتر ممكن يتعلّم عليه. استخدم
     * حساب مخصّص للغرض ده، مش حساب القناة.
     */
    VIDEO_MIRROR_COOKIES: optionalSecret,
     ...
     * يوتيوب بس.
     *
     * ## الافتراضي بقى `false` — ٢٣ سبتمبر ٢٠٢٦
     *
     * كان `true` عشان «السلوك ما يتغيّرش على أي ستاك من غير قرار». القرار
     * اتاخد: صاحب المنصة طلب الرفع المباشر وقال إنه مش عايز السحب من يوتيوب.
     *
     * والواقع بيقول نفس الحاجة. جرّبنا الخمسة كلهم من السيرفر في نفس اليوم —
     * `visionos`، `android_vr`، `tv_simply`، `mweb`، `default` — وكلهم ردوا
     * «Sign in to confirm you're not a bot». يعني الافتراضي `true` مكانش
     * بيشغّل ميزة، كان بيشغّل **مهمة بتفشل كل دقيقة** وبتسيب قطع ناقصة في
     * الباكت. الافتراضي لازم يبقى السلوك اللي بيشتغل، مش اللي المفروض يشتغل.
     *
     * ودي حالة الستاكات التلاتة سوا — هما على **نفس السيرفر** ونفس الـIP
     * المرفوض، فمفيش ستاك كان السحب شغّال عنده وهيتكسر بالتغيير ده.
     *
     * `VIDEO_MIRROR_FROM_YOUTUBE=true` لسه بيرجّعه لأي حد عايزه، و
     * `VIDEO_MIRROR_COOKIES` هو اللي بيخلّيه ينجح ساعتها.
     */
