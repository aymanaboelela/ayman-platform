...
# ── ٣. البيانات المرجعية ──────────────────────────────────────────────
# المحافظات الـ٢٧، النظامين الدراسيين، السنين، المسارات، المواد، ورايات
# الميزات. `migrate deploy` بيعمل الجداول وبس — مبيحطش فيها ولا صف.
#
# ⚠️ ودي مش رفاهية: من غيرها **التسجيل نفسه مكسور**. شاشة الـonboarding
# بتطلب المحافظة والنظام الدراسي، والاتنين بيتقروا من الجداول دي — فقايمة
# فاضية معناها إن الطالب يقدر يعمل حساب ومش يقدر يكمّله. الداتابيز على
# الإنتاج كانت كده بالظبط: سكيما كاملة وصفر بيانات مرجعية.
#
# آمن إنه يشتغل كل مرة: كله `upsert`، و`enabled` بتاعة الرايات بتتحط وقت
# الإنشاء بس — يعني تبديل الأدمن لأي راية بيعيش بعد إعادة التشغيل.
echo "entrypoint: seeding reference data…"
node dist/scripts/seed.js \
  || echo "entrypoint: WARNING — reference-data seed failed" >&2

# ── ٤. حساب الأدمن الأولي ─────────────────────────────────────────────
