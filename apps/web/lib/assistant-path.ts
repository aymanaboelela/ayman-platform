...
function labelFor(path: readonly string[], index: number): string | null {
   ...
   * the honest answer is the node's own id rather than a guess — but the
   * caller decides whether to render that, which is why this returns null and
   * `assistantPathLabels` drops it.
   */
...
/** The same, for the widget's trail — which DOES render the root as its first stop. */
