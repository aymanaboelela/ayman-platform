 ...
 * the student can see the whole of where they are in three lines instead of
...
export function AssistantGuide({
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
              ...
              return (
                <li key={`${path[index]}-${index}`} className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    // as a button that does nothing teaches the student that
                    // buttons here sometimes do nothing.
                    disabled={isCurrent}
                    onClick={() => rewindTo(index)}
                    className={cn(
                      'rounded-full px-2.5 py-1 text-[length:var(--fs-text-xs)] transition-colors duration-[160ms] ease-out',
                      isCurrent
      ...
      >
        <p className="text-[length:var(--fs-text-sm)] leading-[1.75] text-fg">
          {c.script[nodeId]}
        </p>
  ...
  const inner = (
    <>
      <span
        aria-hidden="true"
        className={cn(
          'grid size-8 shrink-0 place-items-center rounded-lg transition-colors duration-[160ms] ease-out',
          isEscalate
      ...
      <span
        className={cn(
          'flex-1 text-[length:var(--fs-text-sm)]',
          isEscalate ? 'font-medium text-fg' : 'text-fg',
        )}
      >
        ...
        <li key={course.id}>
          ...
          >
            ...
            <span className="mt-0.5 block text-[length:var(--fs-text-xs)] text-fg-muted">
              {/* Latin digits, matching `ar-EG-u-nu-latn` — the numbering
                  system every other formatter in this app already uses. */}
