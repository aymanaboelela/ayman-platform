  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const thread = await apiPost('/api/assistant/conversations', ConversationThreadSchema, {
        ...
        // anyway, and sending empty strings would fail the contract's own
        // minimum-length checks for no reason.
       ...
       */
      setError(
        caught instanceof ApiRequestError && caught.status === 429 ? c.tooMany : c.failed,
      );
    } finally {
      setPending(false);
    }
