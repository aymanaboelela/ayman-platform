 ...
 * `admin/nav-items.ts` follows. A `Record` over `AssistantChoiceId` rather
 * than a partial map, so a new branch cannot ship as the one row in the panel
 * with a blank square where every sibling has a glyph.
 *
 * This lives in `apps/web`, not in the contract. The tree is data both apps
 * could read; `lucide-react` is a rendering concern and has no business in a
