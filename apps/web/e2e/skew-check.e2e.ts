 ...
 *
 * `packages/contracts/src/zod.ts` compiles to a client module whose id — 461716
 * as of 2026-08-28 (was 237145 when this test was written; a night of new
 * files added under `packages/contracts/src` shifted it, and it was confirmed
 * stable across two independent clean `next build` runs of the same tree, so
 * this is not per-build noise) — Turbopack derives from the FILE PATH, so it
 * is the same number in every build UNTIL the id-allocation inputs change
 * (e.g. sibling modules being added), and the client runtime keeps the FIRST
 * factory registered for an id and silently drops every later one. That is
 * how the admin course page died on 2026-08-18 at 03:26 with `(0 ,
 * t.partialWithoutDefaults) is not a function`: the helper had just been added
 * as this module's first-ever export, and every browser still holding a chunk
 * from the previous build had the id pinned to the export-less factory.
 *
...
for (const { label, body } of STALE_BUILDS) {
  test(`a tab holding ${label} still opens the course form`, async ({ page }) => {
    await page.addInitScript(
      `(globalThis.TURBOPACK||(globalThis.TURBOPACK=[])).push(["static/chunks/stale-build.js",461716,` +
    ...
    expect(
      ...
      'the stale registration never won — module 461716 is no longer zod.ts, so this test proves nothing',
    ).toBe(true);
