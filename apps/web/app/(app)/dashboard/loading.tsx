 ...
 * the real page's regions IN ORDER — header, hero, stat strip, then the
 * courses/aside split — so the layout does not jump when the two authenticated
 * fetches resolve. Bar widths are varied (full/wide/narrow) rather than
 * uniform, the biggest "cheap skeleton" tell.
 *
 * The hero moved ABOVE the stat strip in the rebuild and this file follows it.
 * A skeleton whose order no longer matches the page it stands in for is worse
 * than none at all: it promises a layout and then rearranges it.
 *
 * One hero block covers both possible occupants — the resume card and the
...
export default function Loading() {
  return (
    <main className="mx-auto w-full max-w-[var(--w-shell)] px-4 py-8 md:px-6 md:py-10">
      ...
      <div className="mb-6 space-y-4 rounded-lg border border-line bg-surface-2 p-5 sm:p-6">
        <Skeleton width="narrow" className="h-3" />
      ...
      <div className="mb-8 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        {Array.from({ length: 4 }, (_, index) => (
          <div key={index} className="space-y-3 rounded-lg border border-line bg-surface-2 p-4">
            ...
            <Skeleton width="full" className="h-[3px]" />
          </div>
        ))}
      </div>
