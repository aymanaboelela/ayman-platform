 ...
 * the real page's regions IN ORDER — band, hero slot, stat strip, achievements,
 * courses — so the layout does not jump when the authenticated fetches resolve.
 * Bar widths are varied (full/wide/narrow) rather than uniform, the biggest
 * "cheap skeleton" tell.
 *
 * ⚠️ A skeleton whose order no longer matches the page it stands in for is
 * worse than none at all: it promises a layout and then rearranges it. Two
 ...
 *    leaves the contents blank, which is what a skeleton is for.
 * 2. The achievements strip sits between the stats and the courses, and it is
 *    six equal cells — the one region whose shape a generic bar cannot suggest.
 *
