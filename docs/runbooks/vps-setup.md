...
### إصلاح أبعاد الصور المتحركة (مرة واحدة)

الرفع كان بيخزّن **طول شريط الفريمات** بدل طول الفريم الواحد في
`media_asset.width/height` — يعني أنيميشن ٣ فريمات ٤٠×٣٠ اتسجّل ٤٠×٩٠. مسار
الكتابة اتصلح، بس الصفوف اللي اتكتبت قبل كده لسه غلط، وأي مكون بيبني
`aspect-ratio` من العمودين دول بيحجز طول أكبر بمرّات وبعدين ينهار عند فك
الترميز — يعني layout shift مصدره الداتابيز.

السكربت بيقرا كل ملف من التخزين ويسأل sharp، لأن مفيش حاجة في الصف بتقول
"متحرك". وبيصلّح الصف **بس** لو الطول المخزّن يساوي `pageHeight × pages`
والعرض متطابق أصلًا؛ أي حاجة متحركة غير كده بيطبعها لك بدل ما يكتب فوقها.

**`--apply` مش موجودة في الأمر الأول عن قصد** — شوف الأرقام الأول:
...
بيطبع سطر زي: `scanned 86 assets — 0 animated, 0 to repair`. لو `to repair`
طلع صفر، **خلصت — مفيش حاجة تتعمل**. لو طلع رقم، شغّله تاني بـ `--apply`.

السكربت idempotent: الصف اللي اتصلح مبيبقاش مطابق للشرط تاني، فإعادة التشغيل
بعد فشل نصّي آمنة.

> ⚠️ ده بيتشغّل **على السيرفر**. الـ deploy بيوصل production عن طريق webhook
> بتاع Dokploy مش SSH، ومفيش كونتينر على جهازك تعمله exec — `docker ps` على
> الماك بيرجع فاضي. ولو نسخت `<api-container>` حرفيًا، zsh بيقرا `<` كإعادة
> توجيه ويرد `no such file or directory: api-container`؛ استخدم صيغة
> `$(docker ps …)` اللي فوق.
