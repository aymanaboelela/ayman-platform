...
**٣. حد الرفع في Cloudflare المجاني ١٠٠ ميجا.** المنصة بتقبل مستندات لحد
٢٠٠ ميجا (`MAX_DOCUMENT_BYTES`). يعني بريزنتيشن أكبر من ١٠٠ ميجا **هيترفض من
Cloudflare** قبل ما يوصل للسيرفر أصلًا، والرسالة هتيجي من Cloudflare مش من
المنصة. اختار واحد:

- نزّل الحد لـ ٩٥ ميجا في `packages/contracts/src/admin/media.ts` (أبسط حاجة)، أو
- سيب `media.aymanaboelela.com` **من غير** proxy (سحابة رمادية) — الرفع بيعدّي
  مباشرة، بس بتفقد حماية Cloudflare على الدومين ده، أو
- اشترك في خطة مدفوعة.
...
| ٥٢٥ من Cloudflare | SSL على "Full (strict)" بس Caddy مجابش شهادة — رجّع رمادي، استنى، ارجع برتقالي |
