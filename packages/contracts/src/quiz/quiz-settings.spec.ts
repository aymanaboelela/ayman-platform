...
describe('review options matrix', () => {
  it('is exactly four windows by seven flags', () => {
    ...
    for (const window of REVIEW_WINDOWS) {
      expect(Object.keys(DEFAULT_REVIEW_OPTIONS[window]).sort()).toEqual([...REVIEW_FLAGS].sort());
    }
  });
  ...
  it('rejects a matrix missing a flag', () => {
    const broken = structuredClone(DEFAULT_REVIEW_OPTIONS) as Record<string, unknown>;
  ...
  it('shows nothing during an attempt', () => {
    expect(Object.values(DEFAULT_REVIEW_OPTIONS.during).every((v) => v === false)).toBe(true);
  });
});

describe('quiz settings defaults', () => {
  it('defaults to a single sitting with no improvement', () => {
    const parsed = QuizSettingsSchema.parse({ reviewOptions: DEFAULT_REVIEW_OPTIONS });
