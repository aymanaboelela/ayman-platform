  ...
  blocks.forEach((lines, index) => {
    ...
    if (declaredType !== null && ORDERING_WORDS.has(declaredType)) {
      ...
      if (options.length === 0) {
        errors.push({
          blockIndex,
          line: 1,
          message: formatCopy(copy.quizErrors.importNoOptions, { n: blockIndex }),
        });
        return;
      }
      if (!sawAnswerLine) {
        errors.push({
          blockIndex,
          line: 1,
          message: formatCopy(copy.quizErrors.importNoAnswerLine, { n: blockIndex }),
        });
        return;
      }
      ...
      candidate = {
        ...
        options: options.map((option, position) => ({
          ...
          fraction: correctIndexes.includes(position) ? (correctIndexes.length > 1 ? share : 1) : 0,
        })),
      };
    }
    ...
    if (!parsed.success) {
      errors.push({
        blockIndex,
        line: 1,
        message: parsed.error.issues[0]?.message ?? copy.quizErrors.importUnknownType,
      });
      return;
    }
