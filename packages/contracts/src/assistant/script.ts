...
export const ASSISTANT_NODES: Record<AssistantNodeId, AssistantNode> = {
  ...
  /*
   * Price used to be a DEAD END that escalated, on the reasoning that only a
   * human knows today's number.
   *
   * There is no number. The courses are free (`Course.priceCents` defaults to
   * 0 and the entitlement check short-circuits on it), which the rest of the
   * product has always said — so the old shape sent students to Ayman's inbox
   * to ask a question the catalog page answers on its own.
   *
   * Now it answers and points at the two things a convinced student does
   * next. Restore the escalation only if a paid course ships AND the price
   * genuinely cannot be stated here.
   */
  joinPrice: {
    choices: [
      { id: 'register', href: '/register' },
