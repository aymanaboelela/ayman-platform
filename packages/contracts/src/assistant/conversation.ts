...
export const MyConversationSchema = z.object({
  ...
  /**
   * Whether the caller has a session, answered HERE rather than by a second
   * request to `/api/profile/me`.
   *
   * It decides one thing: whether the escalation form asks for a name and a
   * phone number. A signed-in student's profile already holds both, and
   * re-asking is the kind of small disrespect that makes a product feel
   * unfinished (`assistant-escalate.tsx` says the same).
   *
   * ⚠️ NOT an authorization signal, and nothing may start treating it as one.
   * The widget is `@Public()` and this is a rendering hint the client could
   * flip in devtools to reveal two extra inputs. Every route that actually
   * matters re-derives the session server-side.
   */
