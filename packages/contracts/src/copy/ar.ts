...
export const copy = {
  ...
  auth: {
     ...
     * دخول ولي الأمر.
     *
     * ⚠️ `failed` **واحدة لكل الأسباب**: كود غلط، حساب متقفل، محاولات
     * خلصت. رسالة بتفرّق بينهم بتقول لأي حد إن الكود ده صح بس الحساب
     * متقفل — وده معلومة عن حساب مش بتاعه، وصّلها له الرفض نفسه.
     */
    guardian: {
      tabStudent: 'طالب',
      tabGuardian: 'ولي أمر',
      hint: 'اكتب الكود اللي في صفحة ابنك على المنصة.',
      codeLabel: 'كود ولي الأمر',
      signIn: 'ادخل',
      signingIn: 'بيدخل…',
      failed: 'الكود مش مظبوط. اطلبه من ابنك تاني.',
      /**
       * القفل بعد خمس غلطات. المدة بتكبر (دقيقة ← ٥ ← ١٥ ← ساعة)، فالصيغ
       * التلاتة كلها بتتوصل — نفس اللي `loginLocked` عامله بالظبط.
       */
      lockedOneMinute: 'الكود اتكتب غلط كذا مرة. استنى دقيقة وجرّب تاني.',
      lockedTwoMinutes: 'الكود اتكتب غلط كذا مرة. استنى دقيقتين وجرّب تاني.',
      locked: 'الكود اتكتب غلط كذا مرة. استنى {minutes} دقايق وجرّب تاني.',
      lockedLong: 'الكود اتكتب غلط كذا مرة. استنى {minutes} دقيقة وجرّب تاني.',
    },
    login: {
      title: 'تسجيل الدخول',
  ...
  landing: {
    ...
    honorBoard: {
       ...
       * is added.
       */
      /* عشرة مش أربعة: اللوحة بقى فيها صف بيتحط بالإيد بمركز مكتوب لحد ١٠
         (`honor_board_pins_rank_range`)، وكارت مركزه ٥ كان بيطلع من غير كلمة
         خالص — `placeRanks[4]` مكانش موجود. الأربعة الأولانيين زي ما هما
         بالحرف. */
      placeRanks: [
        'المركز الأول',
        'المركز التاني',
  ...
  library: {
     ...
     */
    /** العنوان فوق الرسمة. مفيش فيه «{months}» عن قصد — الاسم في السطر اللي
  ...
  bookOrder: {
    ...
    duplicateBody:
      'فيه طلب بنفس الكتب على نفس رقم الموبايل. لو ده هو نفس الطلب، مش محتاج تعمل حاجة — هيوصلك زي ما هو.',
    /* ── الطلب القديم فين دلوقتي ────────────────────────────────────────────
       «لو راح للطباعة تقوله راح للطباعة، والشحن وكده». السؤال من غير السطر ده
       بيسيب الطالب يخمّن هل الأول لسه جاي ولا لأ — واللي ما بيعرفش بيطلب تاني،
       وده بالظبط اللي السؤال موجود يمنعه.
       ⚠️ نفس ألفاظ كرت الطلب في «كتبي» بالحرف («بنجهّزه»، «بنطبعه»، «في
       الطريق») — الطالب هيقرا الاتنين في نفس اليوم، وكلمتين مختلفتين لنفس
       الحالة بيقروا كطلبين مختلفين. */
    ...
    duplicateWhereOther: 'مسجّل عندنا.',
    /** «ك-A3F92C» — نفس الرقم اللي على كرت الشحن وفي أي مكالمة عن الطلب. */
    duplicateRef: 'رقم الطلب: {ref}',
    duplicateCancel: 'تمام، سيبه زي ما هو',
    /** ⚠️ «وأدفع تاني» جوه الزرار نفسه مش تحته. ده الزرار اللي بيكلّف فلوس،
     *   واللي بيضغطه لازم يكون شايف كلمة «أدفع» وهو بيضغط. */
    duplicateConfirm: 'لأ، عايز نسخة كمان وأدفع تاني',
    ...
    /* ── «الإيصال ده اتبعت قبل كده» ──────────────────────────────────────
       يوم ١٧/٠٩ طلع خمس طلبة كل واحد عنده طلبين مدفوعين لنفس الكتاب، وأربعة
       منهم رافعين نفس ملف الصورة بالبايت على الاتنين. مش ناس دفعت مرتين:
       الفورم بيبدأ من الأول لو فتحوا من موبايل تاني أو مسحوا بيانات الموقع،
       وماكانش فيه حاجة في السيرفر بتقولهم.

       ⚠️ الصيغة ما بتتهمش حد. «اتبعتت قبل كده» بتوصف الإيصال مش الشخص،
       و«لو كنت عايز نسخة تانية» بتفتح الباب اللي البيع الحقيقي بيعدّي منه
       بدل ما تقفله في وشه. */
    receiptAlreadyUsed:
  ...
  homework: {
    ...
    /** بتشمل HEIC دلوقتي — صور الآيفون بتتحوّل على السيرفر. الجملة القديمة
     *  كانت بتعدّ الصيغ، والطالبة اللي صوّرت بتليفونها كانت بتقرا «ده مش ملف
     *  صورة» وهي فعلًا صوّرت صورة. */
    badType: 'ده مش ملف صورة. صوّر ورقتك تاني وابعتها.',
   ...
   * صفحة ولي الأمر.
   *
   * النبرة هنا مش نبرة الطالب: الأب مش بيذاكر، هو بيطمن. فمفيش «يلا نكمّل»
   * ولا أي دعوة لفعل — الصفحة بتقول الحقيقة وخلاص، والفعل عند ابنه.
   */
  guardian: {
    eyebrow: 'متابعة ولي الأمر',
    title: 'متابعة الطالب',
    of: 'من',
    noCourses: 'لسه مش مشترك في أي كورس.',
    /** «سلّم ٣ من ٥» — المقام مش زينة: رقم من غير مقام مالوش معنى لحد مش
     *  عارف الكورس فيه كام واجب. */
    homeworkLabel: 'الواجبات',
    needsWork: '{n} محتاج يتعاد',
    quizzesLabel: 'امتحانات دخلها',
    passedOf: 'عدّى في {passed}',
    averageLabel: 'متوسط الدرجات',
    /** مكان المتوسط قبل أول ورقة. «٠٪» بيقرا «امتحن وجاب صفر» — وهي جملة
     *  تانية خالص في بيت. */
    noAverage: 'لسه ماامتحنش',
    papersTitle: 'آخر الامتحانات',
    passed: 'عدّى',
    failed: 'مامشيش',
  },
  ...
  dashboard: {
     ...
     *
     * النبرة مقصودة: الكارت ده مش تحذير ولا إعداد، هو **حاجة تديها لحد**.
     * فالجملة بتقول للطالب يعمل إيه بيه، والتحذير تحت بيقول ليه ماينفعش
     * يتنشر — من غير ما يخوّفه من الحاجة نفسها.
     */
    guardianCode: {
      title: 'كود ولي الأمر',
      lead: 'ادّي الكود ده لوالدك، يدخل بيه ويتابع مذاكرتك ودرجاتك.',
      copy: 'انسخ',
      copied: 'اتنسخ',
      warning: 'الكود ده بيفتح صفحتك كلها — ماتنشروش ولا تبعته لحد غير أهلك.',
    },
    whatsappChannel: {
      title: 'قناة الواتساب',
    ...
    /**
     * «إنت على لوحة الشرف» — الكارت اللي بيقول للطالب إن اسمه اتنشر.
     *
     * اللوحة صفحة عامة مالوش سبب يفتحها، فمن غير الكارت ده (والإشعار اللي
     * جنبه) اسمه ممكن يفضل عليها شهر وهو مايعرفش. والفرحة نفسها هي نص
     * الفيتشر — المدرّس بيحط الاسم عشان الطالب يفرح بيه، مش عشان الأرشيف.
     *
     * ⚠️ مفيش كلمة بتتصرّف للمذكر هنا. «اسمك» و«إنت» مخاطَب من غير وصف،
     * و«مبروك» جامدة — «إنت شاطر» هي اللي ممنوعة.
     *
     * بيظهر على آخر تكريم بس، ولحد أسبوعين. كارت «مبروك» دايم بيبقى أثاث،
     * والأثاث مابيتشافش — نفس القاعدة اللي `instructorMessage` فوق ماشي
     * عليها.
     */
    honorBoard: {
      eyebrow: 'لوحة الشرف',
      /** `{rank}` كلمة زي «المركز الأول» — مش رقم، عشان مايبقاش فيه رقم
       *  لاتيني جوّه سطر عربي على الكارت. */
     ...
     * وفيه شهر تاني اتفتح مش ماسكه.
     *
     * مابيتكلّمش مع حد تاني: اللي اشترك ترم أو سنة مايشوفهوش، واللي مادفعش
     * خالص كمان. الشرط بالتفصيل في `DashboardMonthOfferSchema`.
     *
     * ⚠️ مفيش مذكر هنا. «اشترك» و«مشترك» و«تقدر» التلاتة في ليستة
     * `outreach/compose.spec.ts` بالظبط لأنهم بيكبروا ي في المؤنث، فاللي
     * مستخدم بدالهم أسماء: «الاشتراك»، «اشتراكك الشهري». نفس اللعبة اللي
     * `copy.player.monthOffer` بيلعبها.
     */
    monthNews: {
      eyebrow: 'اتفتح جديد',
      /** `{month}` — عنوان الشهر زي ما المدرّس كتبه («شهر ٢»). */
      titleOne: '«{month}» اتفتح',
      titleMany: 'شهور جديدة اتفتحت',
      /** `{course}` — طالب في كورسين بالشهور لازم يعرف ده بتاع مين. */
      course: 'في «{course}»',
      lead: 'اشتراكك الشهري شغّال — والشهر الجديد بيتفتح بنفس الطريقة.',
      /**
       * اللي اشتراكه خلص. مش نفس الجملة، ومش تجميل: عنده كل الكورس مقفول،
       * و`months` بتبقى **كل** الشهور المفتوحة — فـ«شهر ٢ اتفتح» كانت هتكون
       * كذب، والزرار كان هيقرا كترقية بدل تجديد.
       */
      lapsedEyebrow: 'اشتراك منتهي',
      lapsedTitle: 'اشتراكك خلص',
      lapsedLead: 'الشهور المفتوحة دي، والاشتراك بيرجع بنفس الطريقة.',
      /** على الصف نفسه، لما الشاشة فيها كورس خلص وكورس لسه شغّال — العنوان فوق
       *  مايقدرش يقول الاتنين، فالصف بيقول حالته. */
      lapsedChip: 'اشتراكه خلص',
      /** `{count}` محاضرة جوّه الشهر، أو الجملة اللي بعدها لشهر لسه فاضي —
       *  «٠ محاضرة» بتتقرا كرقم مانزلش. */
      lessons: '{count} محاضرة',
      lessonsEmpty: 'محاضراته بتتجهّز',
      total: 'الإجمالي {price}',
      ctaOne: 'الاشتراك في الشهر ده',
      ctaMany: 'الاشتراك في الشهور دي',
      pending: 'طلب الاشتراك بيتراجع — الشهر بيتفتح أول ما يتأكد.',
  ...
  profile: {
    ...
    /** نفس السبب اللي في `homework.badType` — وعدّ الصيغ هنا كان بيوقع
     *  في نفس الفخ: مستخدم الآيفون بيرفع صورة وبيتقاله إنها مش صورة. */
    photoWrongType: 'ده مش ملف صورة. جرّب صورة تانية.',
    // ── identity ───────────────────────────────────────────────────────
  ...
  notifications: {
     ...
     * عامة مالوش سبب يفتحها، ومفيش حاجة على شاشاته بتتغيّر لما اسمه يتحط.
     *
     * ⚠️ الجملتين مابيخاطبوش الطالب بصيغة — «اسمك» مضاف لمخاطَب مش موصوف،
     * و«مبروك» مش فعل بيتصرّف. «إنت شاطر» هي اللي ممنوعة هنا بالظبط.
     *
     * `{rank}` كلمة زي «المركز الأول» مش رقم، عشان مايبقاش فيه رقم لاتيني
     * جوّه سطر عربي في صف الإشعار.
     */
    honorBoardListed: 'مبروك! اسمك على لوحة الشرف',
  ...
  books: {
    ...
    mine: {
      ...
      /* ── «استلمت الكتاب» ────────────────────────────────────────────────
         الزرار ده بيظهر على الطلب اللي «في الطريق» بس، والطالب هو اللي بيقفله.
         قبل كده الطلب ما كانش بيتقفل غير لما حد في الإدارة يفتكر يعلّمه، يعني
         تاريخ الاستلام كان بيقول «الأدمن ضغط إمتى» مش «الكتاب وصل إمتى».
         واللي بيعرف ده هو اللي ماسك الكتاب.
         الصيغة فعل ماضي عن الطالب نفسه — «استلمت» مش «تأكيد الاستلام» — عشان
         اللي بيضغط بيقول حاجة حصلت، مش بيوافق على إجراء. */
      ...
      /** ⚠️ الفشل هنا مش بيلغي حاجة — الطلب زي ما هو والطالب يقدر يعيد. */
      confirmReceivedFailed: 'مقدرناش نسجّلها دلوقتي. جرّب تاني بعد شوية.',
