...
describe('stream choice ⇄ flags', () => {
  it('maps each choice to the pair it names', () => {
    expect(streamFlagsOf('general')).toEqual({ forGeneral: true, forLanguages: false });
    expect(streamFlagsOf('languages')).toEqual({ forGeneral: false, forLanguages: true });
    expect(streamFlagsOf('both')).toEqual({ forGeneral: true, forLanguages: true });
  });
  ...
  it('reads an impossible pair as both rather than crashing', () => {
    expect(streamChoiceOf({ forGeneral: false, forLanguages: false })).toBe('both');
  });
});
...
describe('the schemas refuse a course or lesson that serves nobody', () => {
  ...
  it('cannot catch a one-sided patch — that is the CHECK constraint the database holds', () => {
    expect(CourseUpdateSchema.safeParse({ forGeneral: false }).success).toBe(true);
  });

  it('does catch an explicit both-false patch', () => {
    expect(
      CourseUpdateSchema.safeParse({ forGeneral: false, forLanguages: false }).success,
    ).toBe(false);
  });
});
