...
/**
 * «شهر جديد اتفتح» — كورس بيتباع بالشهور، والطالب دافع فيه، وفيه شهر مفتوح هو
 * مش ماسكه.
 *
 ...
 *
 * ده لأ. الشريط اللي فوق الداشبورد بيتكلّم مع اللي **دفع**: اللي ماسك شهر
 * دلوقتي، أو اللي اشتراكه **خلص**. اللي عمره ما دفع مابيشوفهوش — الكاتالوج
 * وصفحة الكورس بيكلّموه، والجملة «شهر جديد اتفتح» مالهاش معنى لحد عمره ما
 * اشترك.
 *
 * ## تلات حالات، مش اتنين
 *
 * `monthSliceOf` بيرجّع `everything: false` و`monthIds` فاضية لـ«خلص» و«عمره ما
 * دفع» على السوا، فالعدّ لوحده مايفرّقش بينهم. اللي بيفرّق `slice.lapsed`:
 *
 * | الحالة | `monthIds` | `lapsed` | بيشوف؟ |
 * |---|---|---|---|
 * | ماسك «شهر ١» | فيها | — | ✅ «شهر ٢ اتفتح» |
 * | ترم / سنة / «٣ شهور» **لايف** | — | `everything` | ❌ ماسك كل حاجة أصلًا |
 * | الترم اتقفل عليه | فاضية | `revoked` | ✅ والكوبي بتقول «اشتراكك خلص» |
 * | اشتراك بتاريخ وخلص | فاضية | `expired` | ✅ نفس الكلام |
 * | لسه مابدأش | فاضية | `not_yet_valid` | ❌ دافع وبيستنى تاريخ البداية |
 * | عمره ما دفع | فاضية | `null` | ❌ الكاتالوج بيكلّمه |
 *
 * ⚠️ `revoked` **جوّه** الشرط، وده مش تسيب: «الترم خلص» على المنصة دي اسمه
 * `revoked` مش `expired`. `TermService.setOpen(false)` بيختم `revoked_at` على
 * كل grant بـ`scope: term` في الترم ده في نفس الترانزاكشن — الكوهورت كله مرة
 * واحدة. ومنح الشهور مايقدروش يخلصوا بتاريخ أصلًا: `access_grants_month_open_ended`
 * بيفرض `validUntil = null` عليهم، لأن «شهر ٢» محتوى اتشترى مش تلاتين يوم
 * اتأجروا، وقفلته الوحيدة `revokedAt`.
 *
 * فشرط على `expired` لوحده كان هيبان صح في التست وما يشتغلش ولا مرة على
 * الحقيقي.
 *
 * والتمن إن الريفاند بيشوف الشريط كمان. وده مقبول: اللي اترجّعت له فلوسه يقدر
 * يشترك من جديد فعلًا. `not_yet_valid` هو اللي برّه، لأنه دافع ومستنّي تاريخ —
 * «اشتراكك خلص» غلط في وشه.
 *
 * ⚠️ `slice.lapsed` أول سبب مش لايف بيقابله، والليستة بتوصل **الأجدد الأول**.
 * فاشتراك جديد لسه مابدأش بيغطّي على ترم قديم اتقفل، والشريط مايبانش — وده
 * المقصود: أحدث حاجة عنده هي إنه دافع وبيستنى.
 *
...
export const DashboardMonthOfferSchema = z.object({
  ...
  pending: z.boolean(),
  /**
   * اشتراكه خلص، ومش ماسك ولا شهر لايف دلوقتي.
   *
   * الكوبي مش بتتغيّر شكل، بتتغيّر **معنى**: «شهر ٢ اتفتح» جملة صح لحد ماسك
   * شهر ١، وكذب لحد كل الكورس مقفول عليه — و`months` عنده بتبقى **كل** الشهور
   * المفتوحة، مش الجديد فيهم. فالشريط بيقول «اشتراكك خلص» في الحالة دي.
   *
   * `false` معناه ماسك شهر لايف على الأقل. شوف الجدول فوق.
   */
  lapsed: z.boolean(),
});
