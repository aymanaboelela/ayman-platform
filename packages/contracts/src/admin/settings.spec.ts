...
import { BrandingSchema, ContactSchema, SeoSchema, SiteSettingsSchema } from './settings';
...
describe('SeoSchema', () => {
  ...
  it('caps the title at 70 characters', () => {
    expect(SeoSchema.safeParse({ titleAr: 'ا'.repeat(71) }).success).toBe(false);
  });
});

describe('ContactSchema', () => {
  it('requires E.164 for phone and whatsapp', () => {
    expect(ContactSchema.safeParse({ phone: '01001234567' }).success).toBe(false);
    expect(ContactSchema.safeParse({ phone: '+201001234567' }).success).toBe(true);
  });

  it('refuses an http:// social link — a mixed-content link in the footer is a bug, not a choice', () => {
    expect(ContactSchema.safeParse({ facebook: 'http://facebook.com/x' }).success).toBe(false);
    expect(ContactSchema.safeParse({ facebook: 'https://facebook.com/x' }).success).toBe(true);
  });
});
