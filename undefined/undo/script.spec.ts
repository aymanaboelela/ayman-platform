...
describe("assistant script graph", () => {
  ...
  it("gives every node at least one choice", () => {
    ...
    const empty = nodeIds.filter(
      (id) => ASSISTANT_NODES[id].choices.length === 0,
    );
  ...
  it("links only to internal paths", () => {
    ...
    const external = nodeIds.flatMap((id) =>
      ASSISTANT_NODES[id].choices
        ...
        .filter(
          (choice) =>
            !choice.href.startsWith("/") || choice.href.startsWith("//"),
        )
  ...
  it("uses every choice label it declares", () => {
    ...
    const used = new Set<AssistantChoiceId>(
      nodeIds.flatMap((id) =>
        ASSISTANT_NODES[id].choices.map((choice) => choice.id),
      ),
    );
  ...
  it("answers every node with a non-empty body", () => {
    const blank = nodeIds.filter(
      (id) => copy.assistant.script[id].trim().length === 0,
    );
  ...
  it("offers a way to a human from the root", () => {
    ...
    expect(ASSISTANT_NODES[ASSISTANT_ROOT].choices.some(isEscalateChoice)).toBe(
      true,
    );
  });
});
