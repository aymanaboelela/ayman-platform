...
describe("OnboardingSchema", () => {
  ...
  it("rejects a schoolStream of «both» — a student attends one school", () => {
    expect(
      OnboardingSchema.safeParse(basePayload({ schoolStream: "both" })).success,
    ).toBe(false);
  });
  ...
  it("rejects a motherPhone, which is no longer part of the payload", () => {
    expect(
      OnboardingSchema.safeParse(basePayload({ motherPhone: "01198765432" }))
        .success,
    ).toBe(false);
  });
  ...
  it("accepts a valid grade-1 payload with no track", () => {
    const result = OnboardingSchema.safeParse(
      basePayload({ system: "bacalorya", year: 1 }),
    );
  ...
  it("rejects a grade-1 payload that carries a track", () => {
    ...
    if (!result.success) {
      expect(result.error.issues.some((i) => i.path.includes("trackId"))).toBe(
        true,
      );
    }
  });
  ...
  it("rejects an elective subject when the system is ثانوية عامة", () => {
    ...
    if (!result.success) {
      expect(
        result.error.issues.some((i) => i.path.includes("electiveSubjectId")),
      ).toBe(true);
    }
  });
  ...
  it("rejects a track without a system", () => {
    const result = OnboardingSchema.safeParse(
      basePayload({ year: 2, trackId: "track-1" }),
    );
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues.some((i) => i.path.includes("system"))).toBe(
        true,
      );
    }
  });
  ...
  it("normalises the father's phone to E.164", () => {
    const result = OnboardingSchema.safeParse(
      basePayload({ fatherPhone: "01098765432" }),
    );
    expect(result.success).toBe(true);
    if (result.success) {
  ...
  it("rejects an invalid father's phone as loudly as the student's own", () => {
    expect(
      OnboardingSchema.safeParse(basePayload({ fatherPhone: "123" })).success,
    ).toBe(false);
  });
  ...
  it("rejects a payload carrying an explicit userId", () => {
    const result = OnboardingSchema.safeParse(
      basePayload({ userId: "someone-else" }),
    );
    expect(result.success).toBe(false);
  });
