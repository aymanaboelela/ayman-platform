...
= For*`,
    ...
    if (question.type !== "short_answer")
      throw new Error("expected short_answer");
    expect(question.options.map((o) => o.answerPattern)).toEqual([
      "for",
      "For*",
    ]);
...
ANSWER: B`,
    ...
    expect(result.questions[0]!.stemHtml).toBe(
      "<p>السؤال الأول</p><p>سطر تاني من نفس السؤال</p>",
    );
  });
