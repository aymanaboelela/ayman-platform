...
describe("HomeBlockPropsSchema", () => {
  it("rejects a hero variant missing its required headline", () => {
    expect(
      HomeBlockPropsSchema.safeParse({ type: "hero", items: [] }).success,
    ).toBe(false);
  });

  it("accepts a minimal valid hero variant", () => {
    expect(
      HomeBlockPropsSchema.safeParse({ type: "hero", headlineAr: "مرحبًا بيك" })
        .success,
    ).toBe(true);
  });
});
...
describe("StatsPropsSchema", () => {
  it("rejects 5 stat items — the cap is 4", () => {
    ...
    expect(StatsPropsSchema.safeParse({ type: "stats", items }).success).toBe(
      false,
    );
  });
...
describe("HomeBlockReorderSchema", () => {
  it("rejects a duplicated id", () => {
    ...
    expect(HomeBlockReorderSchema.safeParse({ ids: [id, id] }).success).toBe(
      false,
    );
  });
});
