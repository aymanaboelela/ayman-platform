...
describe("CourseCreateSchema", () => {
  ...
  it("rejects a smuggled instructorId, publishedAt or id", () => {
    for (const extra of [
      ...
      expect(
        CourseCreateSchema.safeParse({ ...validCourse(), ...extra }).success,
      ).toBe(false);
    }
  });
});
...
describe("CourseUpdateSchema", () => {
  it("stays strict after .partial()", () => {
    ...
    expect(CourseUpdateSchema.safeParse({ status: "published" }).success).toBe(
      false,
    );
  });
});
...
describe("CourseStatusPatchSchema", () => {
  it("takes status and nothing else", () => {
    expect(
      CourseStatusPatchSchema.safeParse({ status: "published" }).success,
    ).toBe(true);
    expect(
      CourseStatusPatchSchema.safeParse({ status: "published", title: "x" })
        .success,
    ).toBe(false);
    expect(CourseStatusPatchSchema.safeParse({ status: "live" }).success).toBe(
      false,
    );
  });
});
...
describe("LessonCreateSchema", () => {
  ...
  it("defaults completionMode to manual and position is not accepted at all", () => {
    ...
    expect(LessonCreateSchema.safeParse({ ...base, position: 0 }).success).toBe(
      false,
    );
  });
  ...
  it.each([
    ...
    (field) => {
      const payload = {
        ...base,
        [field]: field === "viewLimit" ? 3 : new Date().toISOString(),
      };
  ...
  it("requires a threshold when completion depends on one", () => {
    expect(
      LessonCreateSchema.safeParse({ ...base, completionMode: "on_view" })
        .success,
    ).toBe(false);
    expect(
      LessonCreateSchema.safeParse({
        ...
        completionMinViewSeconds: 60,
      }).success,
    ).toBe(true);
    expect(
      LessonCreateSchema.safeParse({ ...base, completionMode: "on_pass" })
        .success,
    ).toBe(false);
    expect(
      LessonCreateSchema.safeParse({
...
describe("ReorderSchema", () => {
  ...
  it("rejects duplicates, empties, and anything alongside orderedIds", () => {
    const id = uuid();
    expect(ReorderSchema.safeParse({ orderedIds: [id, id] }).success).toBe(
      false,
    );
    expect(ReorderSchema.safeParse({ orderedIds: [] }).success).toBe(false);
    expect(
      ReorderSchema.safeParse({ orderedIds: [id], sectionId: uuid() }).success,
    ).toBe(false);
  });

  it("rejects non-uuid entries", () => {
    expect(ReorderSchema.safeParse({ orderedIds: ["1", "2"] }).success).toBe(
      false,
    );
  });
});
...
describe("LessonResourceInputSchema", () => {
  ...
  it("rejects javascript: and data: links", () => {
    for (const linkUrl of [
      ...
      expect(
        LessonResourceInputSchema.safeParse({ ...base, kind: "link", linkUrl })
          .success,
      ).toBe(false);
    }
  });
  ...
  it("rejects a document with no file", () => {
    expect(
      LessonResourceInputSchema.safeParse({ ...base, kind: "document" })
        .success,
    ).toBe(false);
  });
...
describe("LessonResourceUpdateSchema", () => {
  it("accepts a title-only patch", () => {
    expect(
      LessonResourceUpdateSchema.safeParse({ title: "اسم جديد" }).success,
    ).toBe(true);
  });

  it("refuses to change kind or payload through a patch", () => {
    expect(LessonResourceUpdateSchema.safeParse({ kind: "link" }).success).toBe(
      false,
    );
    expect(
      LessonResourceUpdateSchema.safeParse({ storageKey: "doc/ab/x.pdf" })
        .success,
    ).toBe(false);
  });
});
...
describe("upload caps stay under the Cloudflare edge limit", () => {
  ...
  it("keeps the document cap below the edge limit, with multipart headroom", () => {
    ...
    expect(CLOUDFLARE_FREE_LIMIT - MAX_DOCUMENT_BYTES).toBeGreaterThanOrEqual(
      4 * 1024 * 1024,
    );
  });
