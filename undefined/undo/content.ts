...
export const streamChoiceOf = (flags: {
  forGeneral: boolean;
  forLanguages: boolean;
}): StreamChoice => {
...
const year1HasNoTrack = (value: {
  year?: number;
  trackId?: string | null;
}): boolean => value.year !== 1 || value.trackId == null;

export const CourseCreateSchema = z
  .object(courseWritableShape)
  .strict()
  .refine(year1HasNoTrack, {
    message: "الصف الأول مالوش مسار",
    path: ["trackId"],
  })
...
export const CourseStatusPatchSchema = z
  .object({ status: CourseStatusSchema })
  .strict();
...
const completionRuleIsCoherent = (value: {
  ...
  if (value.completionMode === "on_view")
    return value.completionMinViewSeconds != null;
  if (
    value.completionMode === "on_grade" ||
    value.completionMode === "on_pass"
  ) {
  ...
  .transform((value, ctx): LessonResourceInput => {
    ...
    if (value.kind === "presentation" || value.kind === "document") {
      ...
      if (value.linkUrl !== undefined || value.url !== undefined) {
        ctx.addIssue({
          code: "custom",
          message: "الملف مايجيش معاه رابط",
          path: ["kind"],
        });
        return z.NEVER;
      }
    ...
    if (value.kind === "video") {
      ...
      const videoExternalId =
        value.url === undefined ? null : extractYouTubeId(value.url);
    ...
    if (value.linkUrl === undefined || !value.linkUrl.startsWith("https://")) {
      ctx.addIssue({
        code: "custom",
        message: "الرابط لازم يبدأ بـ https",
        path: ["linkUrl"],
      });
      return z.NEVER;
    }
    if (value.storageKey !== undefined || value.url !== undefined) {
      ctx.addIssue({
        code: "custom",
        message: "الرابط مايجيش معاه ملف",
        path: ["kind"],
      });
      return z.NEVER;
    }
...
export type LessonResourceUpdateInput = z.infer<
  typeof LessonResourceUpdateSchema
>;
