...
describe("extractYouTubeId — accepted forms", () => {
  it.each([
    ...
    [
      ...
      `https://www.youtube.com/watch?v=${ID}&list=PLabc&index=2&t=42s`,
    ],
...
describe("youTubeEmbedUrl", () => {
  ...
  it("accepts an integer start offset", () => {
    expect(
      new URL(youTubeEmbedUrl(ID, { start: 90.7 })).searchParams.get("start"),
    ).toBe("90");
  });

  it("throws rather than emitting anything derived from a URL", () => {
    expect(() => youTubeEmbedUrl(`https://youtu.be/${ID}`)).toThrow(
      /11-character/,
    );
...
describe("youTubeThumbnailUrl", () => {
  it("points at i.ytimg.com, which is the only image host in the CSP", () => {
    ...
    expect(youTubeThumbnailUrl(ID, "maxres")).toBe(
...
describe("extractDriveFileId", () => {
  ...
  it("rejects a host that merely CONTAINS the real one", () => {
    ...
    expect(
      extractDriveFileId(`https://notdrive.google.com/file/d/${ID}/view`),
    ).toBeNull();
  });
  ...
  it("rejects a path with no id, an unknown product, and a missing /d/ marker", () => {
    ...
    expect(
      extractDriveFileId(`https://drive.google.com/drawings/d/${ID}/edit`),
    ).toBeNull();
    expect(
      extractDriveFileId(`https://drive.google.com/file/${ID}/view`),
    ).toBeNull();
  });

  it("rejects a non-http scheme", () => {
    expect(
      extractDriveFileId(`javascript:alert(1)//drive.google.com/file/d/${ID}`),
    ).toBeNull();
  });

  it("does not let traversal walk out of the id segment", () => {
    expect(
      extractDriveFileId("https://drive.google.com/file/d/../../etc/passwd"),
    ).toBeNull();
  });
});
