...
export const PatternOptionSchema = z
  ...
  .refine(
    (value) =>
      countUnescapedAsterisks(value.answerPattern) <= MAX_PATTERN_WILDCARDS,
    {
...
const ShortAnswerSchema = z
  .object({
    ...
    options: z.array(PatternOptionSchema),
  })
  .refine((value) => countFullCredit(value.options) >= 1, {
...
const EssaySchema = z
  .object({
    ...
    options: z.array(z.never()).default([]),
  })
  ...
  .refine(
    ...
    {
      message: copy.quizErrors.maxWordsBelowMin,
