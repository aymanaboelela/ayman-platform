...
export function isVideoAutoComplete(snapshot: VideoProgressSnapshot): boolean {
  ...
  const positionOk =
    ...
    VIDEO_POSITION_THRESHOLD * snapshot.durationSeconds;
  const watchedOk =
    snapshot.watchedSeconds >=
    VIDEO_WATCHED_THRESHOLD * snapshot.durationSeconds;
 ...
 */
export function videoCompletionFraction(
 ...
 */
export function allowedHeartbeatSeconds(
  claimedDelta: number,
  ...
  const wallClock =
    Math.max(Math.floor(elapsedSeconds), 0) + HEARTBEAT_CLOCK_GRACE_SECONDS;
...
const lessonResourceKindSchema = z.enum([
  "presentation",
  "video",
  "document",
  "link",
